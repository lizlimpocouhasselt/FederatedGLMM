...
lapply(1:nrow(par_settings), function(row){
  ...
  load(file.path(getwd(), 'SIMULATION', 'intermediate_results', 'poisson', 'mv_moment_4_df_x4_x5', sprintf("mv_moment_4_df_%04d_%04d_%04d.RData", iter, m, uniform_cluster_size)))
  set.seed(seed)
  ...
  filename_save <- file.path(getwd(), 'SIMULATION', 'intermediate_results', 'poisson', 'ps4_x4_x5', sprintf("pseudodata_4thmom_%04d_%04d_%04d", iter, m, uniform_cluster_size))
